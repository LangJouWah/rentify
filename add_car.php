...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_add = "INSERT INTO Cars (owner_id, brand, model, year, type, capacity, fuel_type, transmission, price, image, status, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?)";
    $stmt_add = $conn->prepare($sql_add);
    $stmt_add->bind_param("issisisdsss", $owner_id, $brand, $model, $year, $type, $capacity, $fuel_type, $transmission, $price, $image_path, $location);
    if ($stmt_add->execute()) {
            ...
            <div class="bg-gray-900 p-6 rounded-lg shadow max-w-md w-full border border-gray-700">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="location" id="location" class="w-full p-3 border border-gray-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600" value="Manila" required>
                    </div>
                    <div class="mb-6">
